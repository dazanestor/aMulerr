...
export function btihToEd2kHash(btih: string): string | null {
  ...
  if (/^[A-Z2-7]{32}$/.test(value)) {
    // base32 alphabet (our synthetic form). A pure-hex string containing
    // 0/1/8/9 falls through to the hex branch below; a [2-7A-F]-only string is
...
export function fromMagnetLink(magnetLink: string) {
  ...
  return { hash, name, size: parseInt(size) }
}

/**
 * aMule uses 16-byte MD4 hashes (32 hex chars). The synthetic btih magnets we
 * emit pad that hash to 20 bytes, so Radarr/Sonarr derive a 40 hex char
 * "infohash" from the magnet and key their history and blocklist on it. To keep
 * the download-client hash consistent between grab time (the magnet) and queue
 * listing (EC), expose the padded 40-char form to clients and strip it back to
 * the ed2k hash before talking to aMule.
 */
