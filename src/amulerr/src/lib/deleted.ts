...
function resolveDataDir(): string {
  try {
    ...
    console.error(
      `Data dir "${PRIMARY_DATA_DIR}" not writable/available (${err.message}), falling back to ${FALLBACK_DATA_DIR} (not persisted across restarts)`,
    )
...
function saveDeletedHashes() {
  try {
    fs.writeFileSync(
      DELETED_HASHES_FILE,
      JSON.stringify(Array.from(deletedHashes)),
      'utf-8',
    )
  } catch (err: any) {
